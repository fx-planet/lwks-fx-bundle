...
// @Author jwrl
// @Created 2017-10-27
// @see https://www.lwks.com/media/kunena/attachments/6375/Adx_Corners_640.png
// @see https://www.lwks.com/media/kunena/attachments/6375/AlphaCnrSplit.mp4
//-----------------------------------------------------------------------------------------//
// Lightworks user effect Adx_Corners.fx
//
...
float4 ps_open_horiz (float2 uv : TEXCOORD1) : COLOR
{
   ...
   return (uv.x > posAmt) ? tex2D (Fg1Sampler, xy1) : (uv.x < negAmt) ? tex2D (Fg1Sampler, xy2) : EMPTY;
}
...
float4 ps_open_main (float2 uv : TEXCOORD1) : COLOR
{
   ...
   float4 Fgd = (uv.y > posAmt) ? tex2D (HcSampler, xy1) : (uv.y < negAmt) ? tex2D (HcSampler, xy2) : EMPTY;
