...
sampler AmtSampler = sampler_state { Texture = <Amt>; };
...
float4 ps_pinch_in (float2 uv : TEXCOORD1) : COLOR
{
   ...
   return (any (xy > (1.0).xx) || any (xy < (0.0).xx)) ? EMPTY : tex2D (Fg1Sampler, xy);
}
...
float4 ps_main_in (float2 uv : TEXCOORD1) : COLOR
{
   ...
   float4 Fgd = (any (xy > (1.0).xx) || any (xy < (0.0).xx)) ? EMPTY : tex2D (VidSampler, xy);
