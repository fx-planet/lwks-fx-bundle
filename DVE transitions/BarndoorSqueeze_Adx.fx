...
float4 ps_expand_Hf (float2 uv : TEXCOORD1) : COLOR
{
   float amount = Amount - 1.0;
   float negAmt = Amount * Split;
   float posAmt = 1.0 - (Amount * (1.0 - Split));

   float4 Fgnd = (uv.x > posAmt) ? fn_tex2D (s_Super, float2 ((uv.x + amount) / Amount, uv.y))
...
float4 ps_expand_Vf (float2 uv : TEXCOORD1) : COLOR
{
   float amount = Amount - 1.0;
   float negAmt = Amount * (1.0 - Split);
   float posAmt = 1.0 - (Amount * Split);

   float4 Fgnd = (uv.y > posAmt) ? fn_tex2D (s_Super, float2 (uv.x, (uv.y + amount) / Amount))
...
technique Expand_Hf
{
   ...
   pass P_2
   { PixelShader = compile PROFILE ps_expand_Hf (); }
}
...
technique Expand_Vf
{
   ...
   pass P_2
   { PixelShader = compile PROFILE ps_expand_Vf (); }
}
