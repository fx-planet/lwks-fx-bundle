...
DeclareLightworksEffect ("Progress bar", "Matte", "Simple tools", "A simple progress bar generator", CanSize);
...
DeclareEntryPoint (VertGrow)
{
   ...
   float c_ramp = saturate ((Ypos - uv2.y) / Length);
   ...
   float4 Fgnd = tex2D (Fg_0, uv2);

   if (all (uv2 > cropTL) && all (uv2 < cropBR)) { Fgnd = lerp (Fgnd, barC, barC.a); }
   else if (all (uv2 > bordTL) && all (uv2 < bordBR)) { Fgnd = BorderColour; }
...
DeclareEntryPoint (HorizGrow)
{
   ...
   float c_ramp = saturate ((uv2.x - Xpos) / Length);
...
DeclareEntryPoint (VertStatic)
{
   ...
   if (all (uv2 > bar_TL) && all (uv2 < cropBR)) { Fgnd = lerp (Fgnd, barC, barC.a); }
   else if (all (uv2 > cropTL) && all (uv2 < cropBR)) { Fgnd = lerp (Fgnd, fillC, fillC.a); }
   else if (all (uv2 > bordTL) && all (uv2 < bordBR)) { Fgnd = BorderColour; }
...
DeclareEntryPoint (HorizStatic)
{
   ...
   if (all (uv2 > cropTL) && all (uv2 < bar_BR)) { Fgnd = lerp (Fgnd, barC, barC.a); }
   else if (all (uv2 > cropTL) && all (uv2 < cropBR)) { Fgnd = lerp (Fgnd, fillC, fillC.a); }
   else if (all (uv2 > bordTL) && all (uv2 < bordBR)) { Fgnd = BorderColour; }
