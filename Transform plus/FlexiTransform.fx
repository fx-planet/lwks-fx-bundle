...
/**
 Flexible transform is built around the Lightworks mask effect to emulate a
 transform effect.  In addition, antialiasing may be applied to the image as it
 is scaled.  This gives a more natural image softening as the image is enlarged,
 ...
 transform effect the masked foreground alpha channel is used.  This means that
 the drop shadow will only appear where it should and not just at the edge of
 frame, as it does with the Lightworks effect.
