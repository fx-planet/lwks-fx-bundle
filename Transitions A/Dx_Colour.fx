...
//
// This effect is a complete rewrite of my Dip2colour.fx to add the ability to adjust
// effect centering, to improve the linearity of the transition to colour, and to expand
// the colour field modes considerably.
//
// It dissolves through a user-selected colour field from one clip to another.  The
// colour percentage can be adjusted from 0% when the effect perform as a standard
...
// subtle colour blends through the transition.  Transition centering can also be
// adjusted.  Thanks khaver for the idea, although this algorithm and implementation
// is entirely my own.
//
// The colour field can be set up to be a single flat colour or a wide range of
// gradients.  In the gradients that blend to the centre, the centre point is also
// fully adjustable.  Asymmetrical colour transitions can be created by changing
// keyframing of the effect centre, opacity, transition curve, gradient centre and
// colour values.
//
