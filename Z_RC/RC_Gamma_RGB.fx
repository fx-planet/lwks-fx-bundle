...
<
   ...
   string Description = "RC RGB-Gamma";                  // The title
...
float4 main( float2 xy : TEXCOORD1 ) : COLOR
{
   ...
   return  float4 (pow (retval.r, 1/ max (RECEIVING(ChRed) +1 , 1E-6) ),
                   pow (retval.g, 1/ max (RECEIVING(ChGreen) +1 , 1E-6) ),
                   pow (retval.b, 1/ max (RECEIVING(ChBlue) +1 , 1E-6) ),
                   retval.a);
}
