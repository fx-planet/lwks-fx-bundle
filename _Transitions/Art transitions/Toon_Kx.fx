...
// Version history:
//
// Built 2021-12-15 jwrl.
// Built to provide blend support to the Toon transition
//-----------------------------------------------------------------------------------------//

int _LwksEffectInfo
<
   string EffectGroup = "GenericPixelShader";
   string Description = "Toon transition (keyed)";
   string Category    = "Mix";
   string SubCategory = "Art transitions";
   string Notes       = "A stylised cartoon transition for supers and blends";
   bool CanSize       = true;
> = 0;
...
<
   ...
   string Enum = "At start of folded delta key,At start of effect,At end of effect";
> = 1;
...
float4 ps_main (float2 uv1 : TEXCOORD1, float2 uv2 : TEXCOORD2, float2 uv3 : TEXCOORD3) : COLOR
{
   ...
   vidY -= (conv - GetPixel (s_Mix_1, uv3 - LwX) + GetPixel (s_Mix_1, uv3 + LwX));
   ...
   float4 Bgnd = GetPixel (s_Mix_1, uv3);
   float4 retval = lerp (float4 (outlines.xxx, 1.0), Bgnd, Amt);
   float4 Fgnd = GetPixel (s_Mix_2, uv3);
   ...
   Bgnd = Ttype != 1 ? GetPixel (s_Foreground, uv1) : GetPixel (s_Background, uv2);

   return lerp (Bgnd, retval, alpha);
}
