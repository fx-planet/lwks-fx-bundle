...
//-----------------------------------------------------------------------------------------//
// Lightworks user effect KaleidoTurbineMix_Kx.fx
//
...
// nouanda.
//
// This effect has been built from that original.  It is a combination of two previous
// effects, KaleidoTurbineMix_Ax and KaleidoTurbineMix_Adx.
//
// Version history:
//
...
<
   ...
   string Description = "Kaleido turbine mix (keyed)";
   string Category    = "Mix";
   string SubCategory = "Geometric transitions";
   string Notes       = "Breaks the blended foreground into a rotary kaleidoscope pattern";
   bool CanSize       = true;
> = 0;
...
float4 ps_main_F (float2 uv1 : TEXCOORD1, float2 uv3 : TEXCOORD3) : COLOR
{
   ...
   float4 Fgnd = (CropEdges && Overflow (uv1)) ? EMPTY : GetPixel (s_Super, xy2);

   return lerp (GetPixel (s_Foreground, uv1), Fgnd, Fgnd.a * mixval);
}
...
float4 ps_main_I (float2 uv2 : TEXCOORD2, float2 uv3 : TEXCOORD3) : COLOR
{
   ...
   float4 Fgnd = (CropEdges && Overflow (uv2)) ? EMPTY : GetPixel (s_Super, xy2);

   return lerp (GetPixel (s_Background, uv2), Fgnd, Fgnd.a * mixval);
}
